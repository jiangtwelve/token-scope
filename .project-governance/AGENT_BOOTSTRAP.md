...
Metadata: governance_skill=project-governance; governance_version=1.1.0; initialized_at=2026-06-24; project_type=macos-native-swift-app
...
- **Task Plan 红线**：实际开发段内部流程中任何 `acceptance_required: true` 的阶段，**进入编码前必须先在 `processes/tasks/<stage_id>.md` 产出颗粒级 task plan 并经用户明确确认**；未确认 task plan 时不允许出现任何编码 / 新建文件 / 修改既有文件的工具调用。详见 `rules/DEVELOPMENT_PROCESS.md` 的 "Task Plan 前置" 段。
